 ...
 */
	public function send(CakeEmail $email) {
		...
		$headers_str = $this->_headersToString($headers);
		$message = implode("\r\n", (array)$email->message());
                $tmp = CakeSession::read('Emails');
                if (empty($tmp)) $tmp = array();
                $tmp[] = array('headers' => $headers, 'headers_str'=>$headers_str, 'message' => $message);
                return CakeSession::write('Emails', $tmp);
	}
